...
function _clickLlistener(_latLng) {
	...
	if( km || m ){
		...
		// 緯度
		document.getElementById("show_y").innerHTML = latLng.lat().toFixed(5);
		
		// 経度
		document.getElementById("show_x").innerHTML = latLng.lng().toFixed(5);
	}
    //  緯度・経度・高度
	else{
		clearMarkers();
...
function setInfo(ido, keido) {
	...
	// 緯度
	document.getElementById("show_y").innerHTML = ido.toFixed(5);
	
	// 経度
	document.getElementById("show_x").innerHTML = keido.toFixed(5);
	...
	if(isKoudo == true){	
		...
		elevator.getElevationForLocations(positionalRequest, function(results, status) {
			if (status == google.maps.ElevationStatus.OK) {
				...
				if (results[0]) {
					koudo= (results[0].elevation.toString() - 0);
					document.getElementById("show_z").innerHTML = koudo.toFixed(2);
...
function notifyParent() {
	...
	if(altitude){
		window.opener.document.getElementById('var_' + altitude).value  = document.getElementById("show_z").innerHTML;
	}
